...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/example_typo3_extension/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/example_typo3_extension/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/example_typo3_extension/Tests/Unit"
            ;;
    esac
fi
